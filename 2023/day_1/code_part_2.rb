...
def find_digits(string)
  valid_sub_strings(string.chars).map do |sub_string|
    HASH.key?(sub_string) ? HASH[sub_string] : sub_string
  end
end

def valid_sub_strings(array)
  sub_strings(array).select do |sub_string|
    HASH.key?(sub_string) || HASH.value?(sub_string)
  end
end
